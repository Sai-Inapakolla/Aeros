...
public final class Mappings {
  ...
  public static Flight toFlightAggregate(FlightDocument flightDocument) {
    return new Flight(
      new FlightId(flightDocument.getFlightId()),
      new FlightNumber(flightDocument.getFlightNumber()),
      new AircraftId(flightDocument.getAircraftId()),
      new AirportId(flightDocument.getDepartureAirportId()),
      new AirportId(flightDocument.getArriveAirportId()),
      new DurationMinutes(flightDocument.getDurationMinutes()),
      flightDocument.getStatus(),
      new Price(flightDocument.getPrice()),
      new ArriveDate(flightDocument.getArriveDate()),
      new DepartureDate(flightDocument.getDepartureDate()),
      new FlightDate(flightDocument.getFlightDate()),
      null,
      null,
      null,
      null,
      null,
      flightDocument.isDeleted()
    );
  }

  public static FlightDocument toFlightDocument(Flight flight) {
    return new FlightDocument(
      flight.getId().getFlightId(),
      flight.getFlightNumber().getFlightNumber(),
      flight.getAircraftId().getAircraftId(),
      flight.getDepartureAirportId().getAirportId(),
      flight.getArriveAirportId().getAirportId(),
      flight.getDurationMinutes().getDurationMinutes(),
      flight.getStatus(),
      flight.getPrice().getPrice(),
      flight.getArriveDate().getArriveDate(),
      flight.getDepartureDate().getDepartureDate(),
      flight.getFlightDate().getFlightDate(),
      flight.isDeleted()
    );
  }
